...
2. **앱 비밀번호** 입력 (네이버 계정 비밀번호가 아님!)
  1) 네이버 메일에서 '환경설정' > 'POP3/IMAP 설정' > 'IMAP/SMTP 설정'으로 이동하여 'IMAP/SMTP 사용'을 '사용함'으로 변경
   <img width="600" height="273" alt="image" src="https://github.com/user-attachments/assets/3c9224a4-1a6d-4890-a39a-db57a87a35a0" />
  주의사항
    IMAP/SMTP 설정을 위해 네이버 로그인 시 '2단계 인증' 및 **'애플리케이션 비밀번호'**가 반드시 필요합니다.
    '애플리케이션 비밀번호'는 네이버 메일 환경설정 > 보안 > 외부 앱 비밀번호에서 생성할 수 있습니다.
